...
// Uniform resampling along accumulated arc length
SKETCHER_EXPORT int resample_stroke_points(
    ...
    const double min_spacing = std::max(0.5, spacing);
    int out_count = 0;

    Vector2D prev(input_points[0].x, input_points[0].y);
    double carried = 0.0;

    // Always start with first point
    output_points[out_count++] = input_points[0];

    for (int i = 1; i < input_count && out_count < max_output; ++i) {
        Vector2D curr(input_points[i].x, input_points[i].y);
        Vector2D seg = curr - prev;
        double seg_len = seg.length();
        if (seg_len < 1e-6) continue;

        Vector2D dir = seg / seg_len;
        double acc = carried + seg_len;

        while (acc >= min_spacing && out_count < max_output) {
            double t = (min_spacing - carried);
            Vector2D p = prev + dir * t;
            PointData out = input_points[i];
            out.x = p.x; out.y = p.y;
            output_points[out_count++] = out;
            acc -= min_spacing;
            carried = 0.0;
            prev = p; // advance along the segment
        }
    ...
    if (out_count < max_output) {
        output_points[out_count++] = input_points[input_count - 1];
    }

    return out_count;
}

// One Euro filter implementation for 2D points + pressure
...
SKETCHER_EXPORT int one_euro_filter_points(
    ...
    // Defaults
    const double mc = std::max(0.1, min_cutoff);
    const double b = std::max(0.0, beta);
    const double dc = std::max(0.1, d_cutoff);

    PointData prev = input_points[0];
    PointData hat_prev = prev;
    Vector2D d_prev(0, 0);
    double dp_prev = 0.0;

    int out_count = 0;
    output_points[out_count++] = hat_prev;

    for (int i = 1; i < input_count && out_count < max_output; ++i) {
        const PointData& curr = input_points[i];
        double dt = std::max(1e-3, curr.timestamp - prev.timestamp);

        // Derivative of signal
        Vector2D d((curr.x - prev.x) / dt, (curr.y - prev.y) / dt);
        double dp = (curr.pressure - prev.pressure) / dt;

        // Low-pass derivative
        double alpha_d = alpha_from_cutoff(dc, dt);
        d_prev = d_prev * (1.0 - alpha_d) + d * alpha_d;
        dp_prev = (1.0 - alpha_d) * dp_prev + alpha_d * dp;

        // Adaptive cutoff
        double cutoff_pos = mc + b * d_prev.length();
        double alpha_pos = alpha_from_cutoff(cutoff_pos, dt);

        // Filter position
        hat_prev.x = (1.0 - alpha_pos) * hat_prev.x + alpha_pos * curr.x;
        hat_prev.y = (1.0 - alpha_pos) * hat_prev.y + alpha_pos * curr.y;

        // Filter pressure (use dp-based cutoff)
        double cutoff_pressure = mc + b * std::abs(dp_prev);
        double alpha_pressure = alpha_from_cutoff(cutoff_pressure, dt);
        hat_prev.pressure = (1.0 - alpha_pressure) * hat_prev.pressure + alpha_pressure * curr.pressure;
        hat_prev.timestamp = curr.timestamp;
        hat_prev.tiltX = curr.tiltX;
        hat_prev.tiltY = curr.tiltY;
